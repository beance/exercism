...
class Poker
  ...
  ROYAL = { 'A' => 14, 'K' => 13, 'Q' => 12, 'J' => 11 }.freeze

  def initialize(hands)
    scores = hands.map { |hand| find_best_hand(hand) }
    @best_hand = hands.each_with_index.select { |_, i| scores[i] == scores.max }.map(&:first)
  end
  ...
  def find_best_hand(cards)
    ...
    score += sets_of(2) * 10**6
    score += sets_of(3) * 10**8
    score += straight * 10**9
    score += flush * 10**10
    score += full_house * 10**11
    score += sets_of(4) * 10**13
    score + straight_flush * 10**14
  end
